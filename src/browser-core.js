(function () {
  ...
  const SETUP_STEPS = [
    ...
    { id: "controller_select", name: "選擇控制器", requiredCheck: "選擇 Switch 2 Pro 或 Joy-Con 2 握把。", skippableWhen: "不可跳過。", successMessage: "控制器 profile 已載入。", failureFix: "請選擇實際固定在治具上的控制器。" },
    { id: "camera_calibration", name: "鏡頭校正", requiredCheck: "遊戲畫面要清楚入鏡。", skippableWhen: "匯入影片先訓練時可稍後做。", successMessage: "鏡頭已看清楚畫面。", failureFix: "請移動攝影機，讓螢幕四角都在畫面內。" },
    { id: "rig_calibration", name: "完整手把校正", requiredCheck: "每個搖桿、按鍵、肩鍵都要測試到位。", skippableWhen: "不可進入正式遊玩前跳過。", successMessage: "完整手把校正完成。", failureFix: "若按不到，請把對應模組往下或往內調 2-3 mm。" },
    { id: "video_import", name: "影片匯入", requiredCheck: "可匯入遊玩影片做預訓練。", skippableWhen: "沒有影片也可直接實機自學。", successMessage: "影片來源已選擇，等待訓練引擎匯入。", failureFix: "請使用清楚且沒有遮擋的遊戲畫面影片。" },
    { id: "training", name: "訓練", requiredCheck: "讓 AI 先學習基本路線與不要撞牆。", skippableWhen: "正式遊玩前建議至少跑一次。", successMessage: "AI 已有可用的起始策略。", failureFix: "如果沒有進步，請重新校正鏡頭或降低遊戲難度。" },
    { id: "live_play", name: "正式遊玩", requiredCheck: "安全閘門通過後才能啟動。", skippableWhen: "不可跳過安全檢查。", successMessage: "正式遊玩安全檢查已完成。", failureFix: "請確認急停、馬達行程與失聯歸零都正常。" }
  ];

  function createRigConfig(profileId) {
    const profile = CONTROLLER_PROFILES[profileId];
    if (!profile) throw new Error(`Unknown controller profile: ${profileId}`);
    return {
  ...
  function createDefaultActionCommand(rigConfig) {
    return {
      timestamp: Date.now(),
      durationMs: 120,
      priority: "normal",
      sticks: rigConfig.slots.filter((slot) => slot.type === INPUT_TYPES.STICK).reduce((acc, slot) => {
        acc[slot.id] = 0;
        return acc;
      }, {}),
      buttons: rigConfig.slots.filter((slot) => slot.type !== INPUT_TYPES.STICK).reduce((acc, slot) => {
        acc[slot.id] = false;
        return acc;
      }, {})
    };
  ...
  function clampActionCommand(command, rigConfig) {
    ...
    const commandLimitMs = Math.min(rigConfig.safety.maxCommandMs, ...pressedSlotLimits);
    const clamped = {
      ...command,
  ...
  function createNxbtCommand(command) {
    return {
  ...
  function computeLearningScore(gameState) {
  ...
  function shouldRollbackModel({ previousScore, currentScore, policy = DEFAULT_LIVE_POLICY }) {
  ...
  function shouldSwitchToShadowModel({ mainScore, shadowScore, policy = DEFAULT_LIVE_POLICY }) {
